...
def run(submission):
    ...
    for turn in submission['tests']:
        ...
        try:
            sys.stdout = logfile
            bledy = ""
            quest = subprocess.run(["/usr/bin/time","-f","\t%U","-o",os.path.abspath(time_dir + "/" + str(turn['TEST_ID']) + ".log"),"nsjail", "--config", os.path.abspath(str(os.path.dirname(os.path.realpath(__file__))) + "/../sandboxing/policy/pythonexecpolicy.cfg"), "-T", root_dir, "-B", root_dir+"/misc", "-R", code_file, "--cwd", root_dir, "--time_limit", str(math.ceil(turn['max_time'])), "--rlimit_as", str(turn['max_memory']), "--quiet","--", "/usr/local/bin/python3", code_file],stdin=open(input_dir + "/" + str(turn['TEST_ID']) + ".in"),capture_output=True,text=True)
            print(quest.stdout)
