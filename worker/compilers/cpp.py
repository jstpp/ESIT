...
def run(submission):
    ...
    for turn in submission['tests']:
        ...
        try:
            quest = subprocess.run(["nsjail", "--config", os.path.abspath(str(os.path.dirname(os.path.realpath(__file__))) + "/../sandboxing/policy/cppexecpolicy.cfg"), "-T", root_dir, "-B", misc_dir, "-R", code_file, "--cwd", root_dir, "--time_limit", str(math.ceil(config.worker_max_compilation_time)),"--rlimit_as", str(config.worker_max_compilation_memory),"--quiet","--","/usr/bin/g++","-std=c++20","-pedantic","-Wall","-ftime-report","-o",(misc_dir + "/" + str(turn['TEST_ID']) + ".out"),code_file],capture_output=True,text=True)

            sys.stdout = logfile
            quest2 = subprocess.run(["/usr/bin/time","-f","\t%U","-o",os.path.abspath(time_dir + "/" + str(turn['TEST_ID']) + ".log"),"nsjail", "--config", os.path.abspath(str(os.path.dirname(os.path.realpath(__file__))) + "/../sandboxing/policy/cppexecpolicy.cfg"), "-T", root_dir, "-B", misc_dir, "-R", code_file, "--cwd", root_dir, "--time_limit", str(math.ceil(turn['max_time'])),"--rlimit_as", str(turn['max_memory']),"--quiet","--",(misc_dir + "/" + str(turn['TEST_ID']) + ".out"),code_file],stdin=open(input_dir + "/" + str(turn['TEST_ID']) + ".in"),capture_output=True,text=True)
            print(quest2.stdout)
