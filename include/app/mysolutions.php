...
<div class="window">
	...
	<div id="results">
		<?php
			$db_query = $pdo->prepare('SELECT SUBMISSIONS.SUBMISSION_ID AS id, SUBMISSIONS.mode AS mode, SUBMISSIONS.verification_time, SUBMISSIONS.submission_time AS submission_time, SUBMISSIONS.score AS score, SUBMISSIONS.score_percentage AS score_percentage, CONTENT.title AS title, CONTENT.type AS type, CONTENT.maxpoints AS max_pts, CONTENT.CONTENT_ID AS problem_id, CONTENT.result_publish_time AS result_publish_time FROM SUBMISSIONS INNER JOIN CONTENT ON SUBMISSIONS.problem_id=CONTENT.CONTENT_ID WHERE SUBMISSIONS.user_id=:uid ORDER BY SUBMISSIONS.submission_time DESC');
			...
			while($row = $db_query->fetch())
			{
				...
				echo('<a href="index.php?p='.$resultdest.'&sid='.$row['id'].'" class="mysolutions_results_block" style="background-image: '.$gradient.';">
