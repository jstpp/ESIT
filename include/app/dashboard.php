...
<center>
	<h1>Witaj, <?php echo(htmlentities($_SESSION['AUTH_NAME'])); ?>!</h1>
</center>
<div class="window">
...
<div class="window">
	<h2 class="window_title">Najnowsze zbiory zadań</h2>
	<?php
		$db_query = $pdo->prepare('SELECT * FROM PROBLEMSETS ORDER BY SET_ID DESC LIMIT 3;');
		$db_query->execute();

		while($row = $db_query->fetch())
		{
			echo('<a class="set_link" href="?p=set&id='.$row['SET_ID'].'"><i class=\'fas fa-archive\'></i>&emsp;'.$row['title'].'</a>');
		}
	?>
	<br />
</div>
